...
// Read packetId from URL (?packetId=...) or hash (#/tracking?packetId=...) on mount, then auto-track
  useEffect(() => {
    try {
      ...
      if (!fromSearch && window.location.hash) {
        ...
        if (qIndex !== -1) {
          fromHash = new URLSearchParams(hash.slice(qIndex + 1)).get('packetId') || '';
        }
      }
  ...
  // Auto-track when packetId is pre-filled from URL (runs once after mount + state settles)
  const didAutoTrack = React.useRef(false);
  useEffect(() => {
    if (packetId && !didAutoTrack.current) {
      didAutoTrack.current = true;
      handleTrack();
    }
  }); // intentionally no dep array — runs after every render but the ref gates it to once
  ...
  const handleTrack = async () => {
    ...
    try {
      ...
      if (firstStage) setSelectedStage(firstStage.key);
    } catch (err) {
