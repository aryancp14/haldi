...
import { Save, CheckCircle, AlertCircle, ArrowLeft, MapPin, RefreshCw, QrCode } from 'lucide-react';
...
const StageDataEntryForm = ({ onBack }) => {
  ...
  const [createdPacketIds, setCreatedPacketIds] = useState([]);
  ...
  const handleCreatePacketsSubmit = async (e) => {
    ...
    try {
      ...
      setCreatedPacketIds(createdIds);
      setSuccess(`${res.data.message || `Created ${count} packet(s).`}`);
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8">
        ...
        {success && (
          <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg mb-6">
            ...
            {createdPacketIds.length > 0 && (
              <button
                onClick={() => setShowQRModal(true)}
                className="mt-1 inline-flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 transition"
              >
                <QrCode size={16} /> View QR Codes ({createdPacketIds.length})
              </button>
            )}
          </div>
        )}
